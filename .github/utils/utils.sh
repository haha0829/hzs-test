...
check_package_version() {
    ...
    if [[ "$TAG_NAME" == $official_tag && "$TAG_NAME" != $not_official_tag ]]; then
        echo "$(tput -T xterm setaf 1)::error title=Release Version Not Allow::$TAG_NAME does not allow packaging (e.g. v0.1.0)$(tput -T xterm sgr0)"
        exit_status=1
    elif [[ "$TAG_NAME" == $beta_tag ]]; then
        echo "$(tput -T xterm setaf 1)::error title=Beta Version Not Allow::$TAG_NAME does not allow packaging (e.g. v0.1.0)$(tput -T xterm sgr0)"
        exit_status=1
    elif [[ "$TAG_NAME" == $rc_tag ]]; then
        echo "$(tput -T xterm setaf 1)::error title=Release Candidate Version Not Allow:: does not allow packaging (e.g. v0.1.0)$(tput -T xterm sgr0)"
        exit_status=1
    else
